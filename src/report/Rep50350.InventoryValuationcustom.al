report 50350 "Inventory Valuation custom"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'InventoryValuationCustom.rdl';
    ApplicationArea = Basic, Suite;
    Caption = 'Inventory Valuation Custom';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            //GR++
            // These columns are no longer needed here as the quantities will be calculated in the buffer
    ...
    local procedure UpdateBuffer(var ItemLedgEntry: Record "Item Ledger Entry")
    var
    ...
    begin
        ...
        // **NEW LOGIC: Determine the aging bucket and add the quantity to the correct field**
        if ItemLedgEntry."Posting Date" > AgingDates[2] then // 0-6 months
            TempEntryBuffer.Quantity0_6M += ItemLedgEntry."Remaining Quantity"
        else if ItemLedgEntry."Posting Date" > AgingDates[3] then // 6-12 months
            TempEntryBuffer.Quantity6_12M += ItemLedgEntry."Remaining Quantity"
        else // Older than 12 months
            TempEntryBuffer.QuantityOlderThan12M += ItemLedgEntry."Remaining Quantity";
    ...
    local procedure UpdateTempEntryBuffer()
    begin
