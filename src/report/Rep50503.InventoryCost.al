report 50503 "InventoryCost"
{
    DefaultLayout = RDLC;
    RDLCLayout = '.InventoryCost.rdl';
    ApplicationArea = Basic, Suite;
    Caption = 'Inventory Valuation Cost GR';
    ...
    dataset
    {
        dataitem(Item; Item)
        {
            ...
            dataitem(BufferLoop; "Integer")
            {
                ...
                //gr++
                column(InventoryValue_0_6M; TempEntryBuffer.InventoryValue_0_6M) { }
                column(InventoryValue_6_12M; TempEntryBuffer.InventoryValue_6_12M) { }
                column(InventoryValue_12M; TempEntryBuffer.InventoryValue_12M) { }
                //gr--
    ...
    begin
        ...
        //gr++
        AgingDates[1] := EndDate;
        AgingDates[2] := CalcDate('-6M', EndDate);
        AgingDates[3] := CalcDate('-12M', EndDate);
    ...
    var
        //gr++
        InventoryValue_0_6M: Decimal;
        InventoryValue_6_12M: Decimal;
        InventoryValue_12M: Decimal;
    ...
    begin
        ...
        // Original logic to calculate the total inventory value. 
        ...
        // New logic to calculate and assign cost to aging buckets.
        // A temporary CostValue variable is used to avoid code duplication.
        if ShowACY then
            CostValue := ItemLedgEntry."Cost Amount (Actual) (ACY)"
        else
            CostValue := ItemLedgEntry."Cost Amount (Actual)";

        if ItemLedgEntry."Posting Date" >= AgingDates[2] + 1 then
            TempEntryBuffer.InventoryValue_0_6M += CostValue
        else if ItemLedgEntry."Posting Date" >= AgingDates[3] + 1 then
            TempEntryBuffer.InventoryValue_6_12M += CostValue
        else
            TempEntryBuffer.InventoryValue_12M += CostValue;
